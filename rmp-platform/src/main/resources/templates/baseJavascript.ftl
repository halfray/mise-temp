...
var formFields = [
<#assign qfNum_var = enNum >
<#list entity.columns as u >          
    <#if u.xtype?exists>            
    <#if u.xtype.name = 'combobox'>
	 {id: EN[${u_index}], name: EN[${u_index}],hiddenName: EN[${u_index}], fieldLabel:CN[${u_index}],url: '${u.xtype.url}',displayField: '${u.xtype.displayField}',valueField: '${u.xtype.valueField}', xtype: 'selectcombo'}
	<#elseif u.xtype.name = 'datefield'>	
	 {id: EN[${u_index}], name: EN[${u_index}], fieldLabel:CN[${u_index}],format:'Y-m-d', xtype: 'datefield'}		
	<#else>
     {id: EN[${u_index}], name: EN[${u_index}], fieldLabel:CN[${u_index}], xtype: 'textfield'}
	</#if>
	<#else>
     {id: EN[${u_index}], name: EN[${u_index}], fieldLabel:CN[${u_index}], xtype: 'textfield'}</#if><#if (qfNum_var>1)>, </#if><#assign qfNum_var = qfNum_var-1 > 	
</#list>
];	
