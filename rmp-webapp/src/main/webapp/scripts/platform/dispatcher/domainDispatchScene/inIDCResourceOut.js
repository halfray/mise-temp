...
var columns = [new Ext.grid.RowNumberer(),sm,
  {header:'ID',dataIndex:'smInIdcResourceOutId',hidden:true},
...
var reset = new Ext.Button({
   ...
   var grid = new Ext.ux.Grid({
    dataMethod:'smInIDCResourceOutAction.getList',
	viewData:false,
	frame : false,
	border: false,
	columns:columns,
...
function getFormPanel(){
	if(Ext.isEmpty(formPanel)){
		...
		/*var cronexpression = new Ext.ux.form.DateTimeField({
			fieldLabel : '定时方式',
			name:'cronexpression',
			width:150,
			format: 'H:i'
...
function operateDispatch(){
	...
	function onBoSave(){
		M.rpc._call(queryPolicyCallBack,'smInIDCResourceOutAction.getConfigPolicy', '1');
