...
var columns = [new Ext.grid.RowNumberer(),sm,
  {header:'ID',dataIndex:'smInCdnResourceOutId',hidden:true},
...
var reset = new Ext.Button({
   ...
   var grid = new Ext.ux.Grid({
    dataMethod:'smInCDNResourceOutAction.getList',
	viewData:false,
	frame : false,
	border: false,
	columns:columns,
...
function operateDispatch(){
	...
	function onBoSave(){
		M.rpc._call(queryPolicyCallBack,'smInCDNResourceOutAction.getConfigPolicy', '4');
		function queryPolicyCallBack(result){
			if(result){
				...
				if(form.isValid()){
					...
					M.rpc._call(saveDispatchCallBack,'smInCDNResourceOutAction.saveDispatch', {
	...
	function queryPolicyCallBack(result){
		...
		function savePolicy(){
			...
			if(form.isValid()){
				...
				M.rpc._call(savepolicyCallBack,'smInCDNResourceOutAction.savePolicy', {
