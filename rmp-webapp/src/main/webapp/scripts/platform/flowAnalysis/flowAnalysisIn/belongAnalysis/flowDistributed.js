base.portal.flowDistributed = Ext.extend(Main.portal.PortalPage, {
			...
			onPointClick : function(event) {
				...
				var mapData = {};
				mapData.provinceName = event.data.Category;
				mapData.businessDate = this.data.businessDate;
				mapData.operatorName = '中国移动';
				var conf = {
						href : 'portalAssemble.do?portalCode=otherFlowDistributedForBelongIn&uxParams='+ encodeURI(Ext.encode(mapData)),
						text:  '',
						icon: '',
						height : '0.5',
						width : '0.7',
			...
			getMapData : function(data) {
				...
				this.data = data;
				M.rpc._call(Main.fun.Fun(self, self.updateMapData),'flowAnalysisAllAction.getBelongFlowDistributedForIn',{
