...
selectComboBox, topN,
{
	...
	handler : function() {
		Ext.getCmp('#selectComboBox').setValue('domainDNSResolNum');
...
//		var url = this.url.queryList+"?export=true&condition="+this.getParams();
		window.open(url);
//		M.rpc._call("dmmHotdomainSortAction.getExcelHotdomainSortList",{javaClass : 'java.util.HashMap',map : getData()});
//		Ext.Ajax.request({
//		   	url: 'dmmHotdomainSortAction.do',
...
var grid = new Ext.ux.Grid( {
	dataMethod : 'dmmHotdomainSortAction.getHotdomainSortList',
	columns : userColumns,
	border : false,
	frame : false,
	fetchSize : 20,
