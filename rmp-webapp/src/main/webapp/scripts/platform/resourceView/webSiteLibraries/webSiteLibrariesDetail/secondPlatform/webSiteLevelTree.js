...
base.portal.webSiteLevelTree = Ext.extend(Main.portal.PortalPage, {
     ...
     init : function(params) {
		...
		var webLevelColumns = [{
			...
			header : "域名省内数量",
			dataIndex : "provinceDomainNum",
			width : 110
		}, {
			header : "域名省内占比",
			dataIndex : "provinceDomainProportion",
			tpl:getFormatTpl('provinceDomainProportion'),
			width : 110
		}, {
			header : "域名网内数量",
			...
			header : "省内请求次数",
			dataIndex : "provinceReqNum",
			width : 110
		}, *//*{
			header : "省内请求次数占比",
			dataIndex : "provinceReqProportion",
			tpl:getFormatTpl('provinceReqProportion'),
			width : 110
		},*/ {
			header : "本网请求次数",
