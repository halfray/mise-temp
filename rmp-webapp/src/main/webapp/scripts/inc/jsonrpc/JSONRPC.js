/*
 * Java to JavaScript Advanced Object Request Broker
 *
 * Copyright (c) 2005 Michael Clark
...
function JSONRpcClient() {
	var arg_shift = 0, req, _function, methods, self;
 ...
 * 参数3-参数N:远程调用的参数
 */
JSONRpcClient.prototype._call = function(){
	var arg_shift = 0, callBack, method;
	
	//如果参数数量小于1,返回
	if (arguments.length < 1) return;
	
	//如果第一个参数类型为'function',则认为是回调函数
	if (typeof arguments[0] == "function") {
  ...
  //组成request
	var req = this._makeRequest(method, args, callBack);
	...
	if (r) {
		if (r.objectID && r.JSONRPCType == "CallableReference") {
			return new JSONRpcClient(this.serverURL, this.user, this.pass,
					r.objectID, r.javaClass, r.JSONRPCType);
		}
...
JSONRpcClient.makeCallableReference = function(self, value) {
	if (value && value.objectID && value.javaClass
		...
		return new JSONRpcClient(self.serverURL, self.user, self.pass,
				value.objectID, value.javaClass, value.JSONRPCType);
	}
	return null;
};
